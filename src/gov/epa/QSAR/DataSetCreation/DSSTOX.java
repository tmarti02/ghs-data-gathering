...
//				System.out.println(InChiKey2+"\t"+BinaryTox);
	 ...
	 * @param endpoint
	 */
	public static void goThroughToxRecords(RecordsQSAR qsarRecords, Vector<RecordChemReg> recordsChemReg,Vector<RecordDSSTox> recordsDSSTox,String folder,String endpoint,boolean omitSalts) {
...
//					System.out.println("Missing DSSTox record for "+recordChemReg.Top_HIT_DSSTox_Substance_Id);
			...
			qsarRecords.toExcelFile(folder+endpoint+"_recordsQSAR.xlsx");
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @param endpoint
	 */
	public static void goThroughToxRecords2(RecordsQSAR recordsQSAR, Vector<RecordChemReg> recordsChemReg,Vector<RecordDSSTox> recordsDSSTox,String folder,String endpoint, boolean omitSalts) {
		...
		try {//			
			...
			for (RecordQSAR rq : recordsQSAR) {

				//Retrieve RecordChemReg by CAS+name (or by CAS if that fails): 
				RecordChemReg recordChemReg=RecordChemReg.getChemRegRecord(rq,htChemRegCAS_Name,htChemRegCAS,htChemRegName);

				if (recordChemReg==null) {
					rq.usable=false;
					rq.reason="No chemreg match";
					continue;
				}

				if (recordChemReg.Validated==null || recordChemReg.Validated.contentEquals("FALSE")) {
					rq.usable=false;
					rq.reason="Not validated in chemreg";
					continue;
				}
				...
				if (recordDSSTox==null) {
					rq.usable=false;
					rq.reason="No record in DSSTox for this SID";
					continue;				
				}

				setDSSToxData(rq, recordDSSTox);					
				omitBasedDSSToxRecord(rq);
				
				if (!rq.usable) continue;
				
				//Stopgap measure until we have qsarready generation in place:
				if (rq.Structure_SMILES_2D_QSAR==null) {
					if (rq.Structure_SMILES!=null) {
						rq.Structure_SMILES_2D_QSAR=rq.Structure_SMILES;
					}
				}
								
				if (omitSalts) {
					omitBasedOnSmiles(rq,rq.Structure_SMILES);
				} else {
