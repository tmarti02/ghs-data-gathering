...
public class SkinSensitizationEchemportal {
	...
	private String omitBasedOnInterpretation(String omitReason,RecordECHA r) {
		...
		if (!interpretReason.equals("")) {
			omitReason=append(omitReason,interpretReason);
		}
		return omitReason;
	}
...
//					System.out.println("Match:"+r.CAS_final+"\t"+sr.Registry_Number+"\t"+altCAS[i]);
	...
	public void goThroughSkinSensitizationTextFileLLNA(String filepath,String filepathGood,String filepathBad,String scifinderFilePath,String ECHACASlookup) {
		try {
			...
			fwGood.write(RecordEchemportal.getHeader()+"\r\n");
			fwBad.write(RecordEchemportal.getHeader()+"\r\n");
...
//				
	...
	private void omitBasedOnInterpretation(RecordEchemportal r) {
	...
	private void omitBasedOnReliability(RecordEchemportal r) {
	...
	private void omitBasedOnStudyResultType(RecordEchemportal r) {
...
//			System.out.println(r.study_result_type.toLowerCase()+"\t"+est);
	...
	private void omitBasedOnTypeOfStudy(RecordEchemportal r) {
...
//		System.out.println(r.CAS_final+"\t"+r.type_of_study);
	...
	private void fixCASFinal(RecordEchemportal r) {
	...
	private void omitDuplicateRecords(String filepathOld,String filepathNew) {
		...
		try {
			...
			Hashtable<String,Vector<RecordEchemportal>>ht=new Hashtable();
...
//				System.out.println(Line);
				...
				if (ht.get(CAS)==null) {
					...
					Vector<RecordEchemportal>records=ht.get(CAS);
					records.add(r);		
				}
			...
			while(it.hasNext()){
				...
				if (records.size()==1) {
					...
					for (int i=0;i<records.size();i++) {
						RecordEchemportal ri=records.get(i);
